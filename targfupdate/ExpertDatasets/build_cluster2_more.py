...
for i in range(1000):
    ...
    a[i][12] = float(a[i][L[0]*3] +r1*math.cos(theta1))
    a[i][13] = float(a[i][L[0]*3+1]+ r1*math.sin(theta1))
    a[i][14]=2.0
    a[i][15] = float(a[i][L[1]*3] +r2*math.cos(theta2))
    a[i][16] = float(a[i][L[1]*3+1]+ r2*math.sin(theta2))
    a[i][17]=2.0
...
print(a[:2][:])
with open('cluster2_more.pkl', 'wb') as f:
    pickle.dump(a, f)