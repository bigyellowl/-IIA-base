...
for i in range(1000):
    ...
    a[i][20]=2.0
    a[i][21] = float(random.uniform(-1+0.05, 1-0.05))
    a[i][22] = float(random.uniform(-1+0.05, 1-0.05))
    a[i][23]=2.0
    a[i][24] = float(random.uniform(-1+0.05, 1-0.05))
    a[i][25] = float(random.uniform(-1+0.05, 1-0.05))
    a[i][26]=2.0

print(a[:2][:])
with open('3with3_idle3.pkl', 'wb') as f:
    pickle.dump(a, f)