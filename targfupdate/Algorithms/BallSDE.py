...
def loss_mlp(model, gnnmodel, x, marginal_prob_std_func, num_objs, eps=1e-5):
    random_t = torch.ones(x.shape[0], device=device) * 0.01
    # -> [bs, 1]
    random_t = random_t.unsqueeze(-1)
    ...
    perturbed_x = copy.deepcopy(x)
    perturbed_x += z * std
    output = model(perturbed_x.float(), random_t, num_objs)
    bs = random_t.shape[0]
    loss_ = torch.mean(torch.sum(((output * std + z)**2).view(bs, -1), dim=-1))
    return loss_
