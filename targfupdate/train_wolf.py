...
def sample_wolf(radius, num_wolf, sheep_location):
  ls_wolf_states = []

  for i in range(num_wolf):
    while True:
      x = np.random.uniform(-radius, radius, 1)[0] + sheep_location[0]
      y = np.random.uniform(-radius, radius, 1)[0] + sheep_location[1]
      if within_grid(x, y):
  ...
  if os.path.exists(filepath):
    ...
    with tqdm(total=num_samples) as pbar:
      for i in range(num_samples):
        ...
        wolf_states = sample_wolf(radius, num_wolf, sheep_states)
...
dataset = load_or_generate_data("train_wolf2.pkl")
k = num_objs - 1
  ...
  for i, real_data in enumerate(dataloader):
    ...
    with torch.no_grad():
      if (epoch + 1) % visualize_freq == 0:
        with open('score_wolf2.pt', 'wb') as f:
          pickle.dump(score, f)
