...
class MultiAgentEnv(gym.Env):
    ...
    def step(self, action_n):
        ...
        # -------------- Append Action --------------
        self.world.past_action = action_n
        self.world.past_obs = copy.deepcopy(self.world.new_obs)
        self.world.new_obs = [[] for _ in range(len(self.agents))]
        # -------------- Append Action --------------
        ...
        for i, agent in enumerate(self.agents):
            ...
            info = {'individual_reward': self._get_reward(agent), 'eval_reward': self._get_info(agent)['rew']}
            info_n.append(info)
    ...
    def _set_action(self, action, agent, action_space, time=None):
        ...
        if agent.movable:
            ...
            if self.discrete_action_input:
                ...
                if self.discrete_action_space:
                    # using 'discrete_action_space is true!!!!!'
                    agent.action.u[0] += action[0][1] - action[0][2]
                    agent.action.u[1] += action[0][3] - action[0][4]
                    # print(agent.action.u[0],'u0')
                    # print(agent.action.u[1],'u1')
                    d = 5
                else:
